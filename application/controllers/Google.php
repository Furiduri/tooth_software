...
class Google extends EA_Controller {
    ...
    /**
     * Complete synchronization of appointments between Google Calendar and Tooth Software.
     *
    ...
    public static function sync($provider_id = NULL)
    {
        try
        {
            ...
            foreach ($appointments as $appointment)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        {
                            throw new Exception('Event is cancelled, remove the record from Tooth Software.');
                        }

                        // If Google Calendar event is different from Tooth Software appointment then update
                        // Tooth Software record.
                        $is_different = FALSE;
            ...
            // Add Google Calendar events that do not exist in Tooth Software.
            ...
            foreach ($google_events->getItems() as $google_event)
            {
                ...
                // Record doesn't exist in the Tooth Software, so add the event now.
                $appointment = [
     ...
     * documentation of OAuth), every Tooth Software installation should use its own calendar api key. So in every
     * api console account, the "http://path-to-Tooth Software/google/oauth_callback" should be included in an allowed redirect URL.
     */
